...
#awk '{ split($2, targets, ","); if (targets[1] != "-") { printf "%d.%d.%d.%s\n", strtonum("0x" substr($1, 1, 2)), strtonum("0x" substr($1, 3, 2)), strtonum("0x" substr($1, 5, 2)), (targets[1] != "-" ? strtonum("0x" targets[1]) : "0"); } }' isi_hitlist_2023_10_31.fsdb > output_file.txt