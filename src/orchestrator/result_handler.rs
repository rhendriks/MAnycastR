...
pub fn symmetric_handler(
    ...
    worker_stacks: &mut HashMap<u32, VecDeque<Address>>,
)
...
pub fn trace_discovery_handler(
    task_result: TaskResult,
    trace_stacks: &mut HashMap<u32, VecDeque<TraceTask>>,
) -> Result<Response<Ack>, Status> {
    // Get catcher that received the anycast probe reply
    let catcher = task_result.worker_id;

    let trace_tasks: Vec<TraceTask> = task_result.result_list.iter().flat_map(|result| {
        ...
        // Generate an iterator of TraceTasks from TTL = 1 up to hop_count
        (1..=hop_count).map(move |i| TraceTask {
            dst: result.src,
            ttl: i,
