                                ...
                                2 | 4 => { // DNS A record or CHAOS
                                    for dst in &targets.dst_list {
                                        ...
                                        match socket_tx.send_to(&packet, None) {
                                            Some(Ok(())) => sent += 1,
                                            Some(Err(e)) => {
                                                warn!("[Worker outbound] Failed to send DNS packet: {e}");
                                                failed += 1;
                                            },
                                            None => error!("[Worker outbound] Failed to send packet: No Tx interface"),
                                        }
                                ...
                                3 => { // TCP
                                    for dst in &targets.dst_list {
                                        ...
                                        match socket_tx.send_to(&packet, None) {
                                            Some(Ok(())) => sent += 1,
                                            Some(Err(e)) => {
                                                warn!("[Worker outbound] Failed to send TCP packet: {e}");
                                                failed += 1;
                                            },
                                            None => error!("[Worker outbound] Failed to send packet: No Tx interface"),
                                        }
...
pub fn _trace_outbound (
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            'outer: loop {
                ...
                loop {
                    match outbound_rx.try_recv() {
                        ...
                        Err(e) => {
                            if e == TryRecvError::Disconnected {
                                error!("[Worker outbound] Channel disconnected");
                                break 'outer;
                            }
                            // wait some time and try again
                            sleep(Duration::from_millis(100));
            ...
            info!("[Worker outbound] Outbound thread finished - packets sent : {}, packets failed to send: {}", sent.with_separator(), failed.with_separator());
        })
        .expect("Failed to spawn outbound thread");
